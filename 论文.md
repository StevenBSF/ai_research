...
where  $g()$和$h()$为view-specific function，$\delta()$和$\zeta()$为 noise corruption function，$c^v$、$s^v$、$u^v$分别为各个视图下的content、style、view-specific的冗余信息,且$c^v$、$s^v$、$u^v$满足$c^v \perp\!\!\!\perp s^v \perp\!\!\!\perp u^v$。
