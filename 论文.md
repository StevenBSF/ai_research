...
我们提出了一个新颖的multi-view clustering model named Multi-Stage Causal Feature Learning for Multi-View Clustering.我们创新性的将causal inference的概念引入multi-view clustering这项任务中，并且在Causal Content-Style Feature阶段，我们分别从intra-view和cross-view的角度对于多视角特征利用causal mechanism层面进行分析，旨在寻求对于视角内部和不同视角之间的聚类内容一致性。在Graph-based Method Contrasive Learning阶段，我们通过利用在上一个阶段经过高斯分布采样的特征与 Preliminary Feature构建similarity graph和pseudo label graph进行对比学习。实验证明了我们的模型在多个数据集上的优越性和有效性。
